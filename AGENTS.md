...
- 本仓库当前仅用于本地课程开发，允许在 `entry/src/main/ets/config/` 集中保存讯飞开发凭证。
- 凭证不得散落在页面、ViewModel、测试或其他业务模块中。
- 日志、错误提示、测试输出不得打印密钥、完整 Authorization 或签名结果。
- 仓库不得在包含真实凭证时推送、公开发布或交付第三方；执行这些操作前必须先移除凭证并完成轮换。
- `.env.example` 只保存变量名和占位符，不能保存真实凭证。
- 修改凭证值、`.env` 或 token 仍必须先问 Champ。
...
- 讯飞凭证只存在于集中配置模块，且没有进入日志、测试输出或其他业务代码。
- 项目能构建。
- 核心测试通过。
